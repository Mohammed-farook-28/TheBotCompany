  ...
  return (
    ...
    >
      <GlowingEffect className="w-full max-w-2xl mx-auto">
        ...
        {!isCompleted && (
          <div className={`px-4 md:px-8 pb-6 md:pb-8 ${footerClassName}`}>
            <div className={`mt-6 md:mt-10 flex ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`}>
              {currentStep !== 1 && (
                <button
                  onClick={handleBack}
...
function StepContentWrapper({ isCompleted, currentStep, direction, children, className }: {
  ...
  const [parentHeight, setParentHeight] = useState<string | number>('auto');
    ...
    >
      <motion.div
        variants={{
          ...
          active: { scale: 1, backgroundColor: '#00baff', color: '#00baff' },
          complete: { scale: 1, backgroundColor: '#00baff', color: '#00baff' }
        }}
...
function StepConnector({ isComplete }: { isComplete: boolean }) {
  const lineVariants = {
    ...
    complete: { width: '100%', backgroundColor: '#00baff' }
  };
